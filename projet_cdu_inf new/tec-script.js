...
const currentDepartment = 'TEC';

// Initialize the application
document.addEventListener('DOMContentLoaded', function() {
    initializeTecApplication();
});

function initializeTecApplication() {
...
function addInstallationRow(trainName = '') {
    ...
    row.innerHTML = `
        ...
        <td>
            <input type="number" id="jours_arret_tdd_${rowIndex}" name="jours_arret_tdd" min="0" placeholder="TDD Days" disabled>
        </td>
        <td>
            <input type="number" id="nbr_declenchement_${rowIndex}" name="nbr_declenchement" min="0" placeholder="Count" disabled>
        </td>
        <td>
            <select id="tdd_${rowIndex}" name="tdd_situation" disabled>
                <option value="">-</option>
                <option value="MNT">MNT</option>
            </select>
        </td>
        ...
        <td>
            <select id="gt_${rowIndex}" name="gt_situation" class="tec-field">
                <option value="">-</option>
                <option value="TEC">TEC</option>
            </select>
        </td>
        <td>
            <input type="text" id="observation_${rowIndex}" name="observation" placeholder="Quelques explications ou observations TEC" maxlength="150" class="tec-field">
        </td>
    `;
